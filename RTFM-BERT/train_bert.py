...
class RTFM_loss(torch.nn.Module):
    ...
    def forward(self, score_normal, score_abnormal, nlabel, alabel, feat_n, feat_a, scores2):
        ...
        #print("loss1 = {}, loss2 = {}".format(loss_cls,loss_cls2)) 
        ...
        if not scores2 is None:
            loss_total = self.beta*loss_cls + (1-self.beta)* loss_cls2 + self.alpha * loss_rtfm
        else: 
...
def train(nloader, aloader, model, batch_size, optimizer, device):
    with torch.set_grad_enabled(True):
        ...
        #ninput = ninput[:,0:1] #use one crop 
        #ainput = ainput[:,0:1]

        #print(ninput.shape,ainput.shape)      
        ...
        feat_normal_bottom, scores, scores_nor_bottom, scores_nor_abn_bag, _, _, _, scores2 = model(inputs)  # b*32  x 2048

        #print(score_abnormal.shape,score_normal.shape) 
        ...
        #print("abn_scores.shape = ", abn_scores.shape) 

        #print("score_normal.shape = ", score_normal.shape) 
        #print("score_abnormal.shape = ", score_abnormal.shape) 
        ...
        loss_criterion = RTFM_loss(0.0001, 100, 1.0)
        loss_sparse = sparsity(abn_scores, batch_size, 8e-3)
        loss_smooth = smooth(abn_scores, 8e-4)
        cost = loss_criterion(score_normal, score_abnormal, nlabel, alabel, feat_select_normal, feat_select_abn, scores2) +  1* (loss_smooth + loss_sparse)
